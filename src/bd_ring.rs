...
pub(super) struct AxiDmaBdRing {
    ...
    #[allow(unused)]
    cyclic: usize,
    pin_buf: Option<Pin<&'static mut [u8]>>
}

impl AxiDmaBdRing {
    pub fn new(config: AxiDmaBdRingConfig, pin_buf: Option<Pin<&'static mut [u8]>>) -> Self {
        Self {
            config,
            is_halted: true,
            ring: VecDeque::new(),
    ...
    pub fn tx_submit<B>(&mut self, buffer: &Pin<B>)
    where
        B: Deref,
        B::Target: AsRef<[u8]> + 'static,
    {
        let bd_len = self.config.max_transfer_len;
        let total_buf_len = (**buffer).as_ref().len();
        
        let total_bd_cnt = (total_buf_len + bd_len - 1) / bd_len;
        trace!(
            "bd_ring::tx_submit: total_buf_len: {}, total_bd_cnt: {}",
            total_buf_len, total_bd_cnt
        );
        // loop wait
        while total_bd_cnt > self.free_cnt {
            // error!("bd_ring::tx_submit: too many BD required! total_bd_cnt {}, free_cnt {}", total_bd_cnt, self.free_cnt);
        }
        ...
        trace!(
            "bd_ring::tx_submit: buf_len: {}, bd_cnt: {}, restart: {}",
        ...
        trace!(
            "bd_ring::tx_submit: done, restart: {}, tail: {}, free: {}, pending: {}",
    ...
    pub fn rx_submit(&mut self) {
        let buf = &self.pin_buf.as_mut().unwrap();
        let start = self.bd_restart;
        let mut buf_len = buf.len();
        let mut buf_head = 0;
        let mut bd_len = self.config.max_transfer_len;
        let bd_cnt = (buf_len + bd_len - 1) / bd_len;
        while bd_cnt > self.free_cnt {
    ...
    pub fn rx_from_hw(&mut self) -> Option<Vec<Pin<&'static [u8]>>> {
        let mut bd_cnt = 0;
        let mut partial_cnt = 0;
        let mut cur_bd = self.bd_head;
        trace!(
            "bd_ring::rx_from_hw: head: {}, tail: {}",
            self.bd_head, self.bd_tail
        );
        compiler_fence(SeqCst);
        fence(SeqCst);
        ...
        loop {
            ...
            if status.cmplt().is_false() {
                // unsafe { ebreak() };
                trace!("bd_ring::rx_from_hw: Uncompleted BD found at {}", cur_bd);
                bd.dump();
                break;
            }
            bd_cnt += 1;
            let ctrl = bd.desc.control.read();
            if ctrl.eof().is_true() || status.rxeof().is_true() {
                trace!("bd_ring::rx_from_hw: EOF found at {}", cur_bd);
                partial_cnt = 0;
            } else {
                partial_cnt += 1;
            }
        ...
        trace!(
            "bd_ring::rx_from_hw: bd_cnt: {}, partial: {}",
            bd_cnt, partial_cnt
        );
        bd_cnt -= partial_cnt;
        if bd_cnt > 0 {
            ...
            warn!("bd_ring::rx_from_hw: no completed BD!");
            None
        }
    }
}